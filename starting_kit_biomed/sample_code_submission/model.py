...
class model:
    def __init__(self):
        ...
        self.mod=RandomForestClassifier(n_estimators=100,bootstrap= False,min_samples_leaf= 1, 
                                        criterion="gini",min_samples_split= 100
                                        , max_features= 3, max_depth= None)
        self.mymodel = Pipeline([
...
class Predictor(BaseEstimator):
    ...
    pour lequel on choisit les hyper-param̬tres.  """
    def __init__(self):
        '''cette methode initialise le classifieur '''
        self.mod = RandomForestClassifier(n_estimators=100,bootstrap= False,  criterion="gini", max_features= 5, max_depth= None)
        print("PREDICTOR=" + self.mod.__str__())

    def fit(self, X, y):
        ''' This is the training method: parameters are adjusted with training data.'''
        self.mod = self.clf.fit(X, y)
        return self
